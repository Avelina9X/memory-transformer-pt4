...
class BoolQInstructDataset( BaseChoiceInstructDataset ): # TODO: refactor as part of SuperGLUE
...
class CopaInstructDataset( BaseChoiceInstructDataset ): # TODO: refactor as part of SuperGLUE
    def __init__( self, cache_dir: str ):
        self.metric = load_metric( 'accuracy', download_config=DownloadConfig( cache_dir=cache_dir ) )
        super().__init__( cache_dir )
    ...
    def _format_single_target( self, doc: dict ) -> Message:
        return Message(
            role='assistant',
            content=[ doc['choice1'], doc[ 'choice2' ] ][ doc['label'] ],
            complete=True,
        )
...
DIRECTORY: Mapping[str, Callable[[str], BaseChoiceInstructDataset]] = {
    'boolq': BoolQInstructDataset,
    'copa': CopaInstructDataset,
}
...
def main():
    ...
    BoolQInstructDataset( cache_dir )
    CopaInstructDataset( cache_dir )
